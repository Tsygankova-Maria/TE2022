...
#endif
void rotate_left(){
 move(3, 80/ 100.0 *255);//65
}
void rotate_right(){
 move(4, 80 / 100.0 * 255);//65
}
void _loop() {
  Encoder_1.loop();
  Encoder_2.loop();
...
int rpi4=A11;*/
  ...
  Serial.println(digitalRead(A13)+digitalRead(A6)*2);
   if((digitalRead(A13)+digitalRead(A6)*2>0) && (digitalRead(A11)==0 && bitRead(mymap[cord_x][cord_y].wall,5)==0)){
       move(1,0);
       vict=0;
       _delay(2);
       for(y = 1; y <= 10; y++){
...
void alignment(){ // выравнивание
  ...
  move(2,100); _delay(0.1); move(1,0);
    }
  ...
  move(1,100); _delay(0.1); move(1,0);
     }
  ...
  move(1,100); _delay(0.2); move(1,0);
  }
    dis_front_read();
    dis_right_b = IR(A10);
    dis_right =  IR(A9);
  ...
  move(2,100); _delay(0.2); move(1,0);
  }
