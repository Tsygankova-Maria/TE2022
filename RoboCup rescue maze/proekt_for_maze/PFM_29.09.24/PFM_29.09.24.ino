...
void abyss(){
 ...
 if(BlackV > V){
    ...
    while(abs(Encoder_1.getCurPos()-Encoder1)<105 && abs(Encoder_2.getCurPos()-Encoder2)<105){
     move(1,180);
     _delay(0.14); 
    }
    move(1,0);
    ...
    if(napravlenie==1) mymap[cord_x-1][cord_y].count=80;
     else if(napravlenie==2) mymap[cord_x][cord_y-1].count=80;
     else if(napravlenie==3) mymap[cord_x+1][cord_y].count=80;
     else mymap[cord_x][cord_y+1].count=80;
...
int rpi4=A11;*/
  ...
  Serial.println(digitalRead(A13)+digitalRead(A6)*2);
   if((digitalRead(A13)+digitalRead(A6)*2>0) && (digitalRead(A11)==0)){
       move(1,0);
       _delay(2);
       for(y = 1; y <= 10; y++){
        for ( i = 1; i <= AURIGARINGLEDNUM; i++ )
...
void alignment(){ // выравнивание
  ...
  if(dis_left<=stena && dis_left_b<=stena  && dis_front>=stena)
    ...
    dis_front_read();
  dis_right =  IR(A9); 
    if (dis_right<=5  && dis_front>=stena)
      while(dis_right<7){
        rotate_right();
       _delay(0.14);
      dis_front_read();
...
void gyro_y(){
 ...
 if(gyro_y < -10 && gyro_y > -40){
  ...
  dis_right =  IR(A9); 
    dis_left_b = IR(A15);
    dis_right_b = IR(A10);
  while(dis_left_b<=stena && dis_right_b<=stena){
   move(2,200);
   _delay(0.14);   
   alignment();
   move(2,0);
   ...
   gyro_y = gyro_0.getAngle(1);  
    ...
    if(napravlenie==1) { mymap[cord_x-1][cord_y].count=80; cord_x--;}
     else if(napravlenie==2) {mymap[cord_x][cord_y-1].count=80; cord_y--;}
     else if(napravlenie==3) {mymap[cord_x+1][cord_y].count=80; cord_x++;}
     else {mymap[cord_x][cord_y+1].count=80; cord_y++;}
  }
 }
 else if(gyro_y > 15){
   dis_left =  IR(A14);
...
/*    if(w -u >=40){
  ...
  if(mymap[cord_x][cord_y].count>1){
      if(mymap[cord_x][cord_y].wall[0]==0 && mymap[cord_x-1][cord_y].count==0) {
        puti=0;
        if(tam_r==1) while(napravlenie!=1) gyro_rotate(1);
        else while(napravlenie!=1) gyro_rotate(0);
        puti=1;
      }
      else if(mymap[cord_x][cord_y].wall[1]==0 && mymap[cord_x][cord_y-1].count==0){
        puti=0;
        if(tam_r==2) while(napravlenie!=2) gyro_rotate(1);
        else while(napravlenie!=2) gyro_rotate(0);
        puti=1;
      }
      else if(mymap[cord_x][cord_y].wall[2]==0 && mymap[cord_x+1][cord_y].count==0){
        puti=0;
        if(tam_r==3) while(napravlenie!=3) gyro_rotate(1);
        else while(napravlenie!=3) gyro_rotate(0);
        puti=1;
      }
      else if(mymap[cord_x][cord_y].wall[3]==0 && mymap[cord_x][cord_y+1].count==0){
        puti=0;
