...
int main(int argc, char **argv)
{
    ...
    mazeBot = new MazeBot(supervisor);  // Передаем супервизор в MazeBot
    
    // Проверяем, что контроллер действительно работает в режиме супервизора
    if (supervisor->getSupervisor()) {
        robot_node = supervisor->getFromDef("JEREMY");  // Теперь это разрешено
    } else {
        std::cerr << "ERROR: This controller must be run as a Supervisor!" << std::endl;
...
void search(MazeGraph &graph,vector<vector<Cell>> &map,Coordinates start,Coordinates goal,unordered_map<Coordinates, Coordinates, HashFunction> &came_from,unordered_map<Coordinates, double, HashFunction> &cost_so_far, stack<Coordinates> &path)
{
    ...
    while (!frontier.empty()){
       Coordinates current = frontier.get();
        if (matchGoal(Coordinates{current.x, current.y}, goal)){G = current;break;}
    ...
    while (!(current == start)){ path.push(current);current = came_from[current];}
    path.push(current);
}
