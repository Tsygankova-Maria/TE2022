...
void okruga(){
   ...
   /* if(mymap[cord_x][cord_y]->count>100 || mymap[cord_x][cord_y]->count<0) mymap[cord_x][cord_y]->count=1;
    else mymap[cord_x][cord_y]->count+=1;*/

    if (dis_front<=stena) mymap[cord_x][cord_y]->wall_up = 1;
    if(dis_right<=stena && dis_right_b<=stena) mymap[cord_x][cord_y]->wall_right = 1;
    if (dis_left<=stena && dis_left_b<=stena) mymap[cord_x][cord_y]->wall_left = 1;
    if (mymap[cord_x][cord_y]->tile == ' ' ) mymap[cord_x][cord_y]->tile = '0';
